...
export default function Model(props) {
  ...
  // Memoize position, scale, rightValue based on window size
  const { posX, posY, scale, rightValue } = useMemo(() => {
    if (size.width <= 400) {
      return { posX: -1.3, posY: -0.4, scale: 0.12, rightValue: 'false' };
    } else if (size.width <= 500) {
      return { posX: -1.2, posY: -0.4, scale: 0.15, rightValue: 'false' };
    } else if (size.width < 860) {
      return { posX: -1.3, posY: -0.3, scale: 0.18, rightValue: 'false' };
    } else if (size.width < 940) {
      return { posX: -1.2, posY: -0.4, scale: 0.18, rightValue: 'false' };
    } else if (size.width < 1200) {
      return { posX: -2.6, posY: 0.2, scale: 0.2, rightValue: 'true' };
    } else if (size.width < 1400) {
      return { posX: -2.8, posY: 0.02, scale: 0.24, rightValue: 'true' };
    } else {
      return { posX: -2.85, posY: 0.14, scale: 0.24, rightValue: 'true' };
    }
  }, [size.width]);
